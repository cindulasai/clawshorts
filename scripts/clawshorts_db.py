...
def _safe_device_column(column: str) -> str:
    """Return a whitelisted SQL column identifier.

    SQLite parameter binding cannot bind identifiers, so any dynamic column name
    must come from this fixed allowlist before it is interpolated into SQL.
    """
    if column not in DEVICE_UPDATE_COLUMNS:
        raise ValueError(f"Unsupported device column: {column}")
    return column
...
def update_device(ip: str, **kwargs: any) -> dict | None:
    ...
    updates = {k: v for k, v in kwargs.items() if k in DEVICE_UPDATE_COLUMNS}
    if not updates:
        return get_device(ip)
    set_clause = ", ".join(f"{_safe_device_column(k)}=?" for k in updates)
