...
validate_ip() {
    local ip="$1"
    # Pass IP as argv, never interpolate it into Python code.
...
while true; do
    for ip in $(echo "$IPS" | tr ',' ' '); do
        ...
        if adb -s "$ip:5555" shell dumpsys activity activities 2>/dev/null | grep -q youtube; then
            ...
            ui_tmp=$(mktemp "${TMPDIR:-/tmp}/clawshorts-ui.XXXXXX.xml")
            trap 'rm -f "$ui_tmp"' RETURN
            adb -s "$ip:5555" shell uiautomator dump /sdcard/ui.xml 2>/dev/null
            adb -s "$ip:5555" pull /sdcard/ui.xml "$ui_tmp" 2>/dev/null

            pct=$(python3 - "$ui_tmp" <<'PY' 2>/dev/null || echo "100"
import re
import sys
try:
