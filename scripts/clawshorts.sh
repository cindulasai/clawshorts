...
# Resolve launcher links to get the real script location
_real_script() {
    local src="$1"
    while [[ -L "$src" ]]; do
        src="$(readlink "$src")"
...
# Hermes-local install: prefer Hermes' venv Python because it already ships
# pydantic v2 and avoids modifying system Python.
...
run_python_cli() {
    local cmd="$1"
    shift
    # Ensure SQLite schema exists before read-only commands like list/status.
    PYTHONPATH="${SCRIPT_DIR}/../src:${SCRIPT_DIR}" "$PYTHON_BIN" -c 'import clawshorts_db as db; db.init_db()' 2>/dev/null || true
    PYTHONPATH="${SCRIPT_DIR}/../src" "$PYTHON_BIN" "$CLAWSHORTS_PY" "$cmd" "$@"
}
...
install_adb() {
    echo "ADB is required but is not installed."
    echo "Install Android platform-tools using your OS package manager, then rerun this command."
    echo "Manual instructions: https://developer.android.com/tools/adb"
    return 1
}
...
cmd_start() {
    ...
    PYTHONPATH="$SCRIPT_DIR/../src" exec "$PYTHON_BIN" "$SCRIPT_DIR/clawshorts-daemon.py"
}
...
cmd_install() {
    # Create user-local bin launcher link so `shorts` works from anywhere without a global install.
    local bin_dir="${HOME}/.local/bin"
    local launcher link="${bin_dir}/shorts"
    mkdir -p "$bin_dir"
    if [[ ! -L "$launcher link" ]] && [[ ! -e "$launcher link" ]]; then
        if ln -s "$SCRIPT_DIR/clawshorts.sh" "$launcher link" 2>/dev/null; then
            echo "✅ Linked: shorts → $launcher link"
        else
            echo "⚠️  Could not create $launcher link — run directly: $SCRIPT_DIR/clawshorts.sh"
        fi
    elif [[ -L "$launcher link" ]]; then
        ln -sf "$SCRIPT_DIR/clawshorts.sh" "$launcher link" 2>/dev/null || true
    fi
    case ":${PATH}:" in
        *:"${bin_dir}":*) ;;
        *) echo "ℹ️  Add ${bin_dir} to PATH if 'shorts' is not found in new shells." ;;
    esac
...
[Service]
Type=simple
ExecStart=${PYTHON_BIN} ${SCRIPT_DIR}/clawshorts-daemon.py
Restart=on-failure
RestartSec=10
