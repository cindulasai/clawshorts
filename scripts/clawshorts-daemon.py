...
from clawshorts.detection import is_shorts_element
...
ADB_BIN: Final = (
    shutil.which("adb")
    or str(Path.home() / ".local" / "bin" / "adb")
    or "adb"
)
...
def _load_devices_from_db() -> list[dict]:
    """Load enabled private-LAN device dicts from SQLite."""
    db.init_db()
    devs = db.get_devices()
    valid: list[dict] = []
    for dev in devs:
        ...
        if not validate_ipv4(ip):
            logging.getLogger("clawshorts-daemon").warning("Skipping invalid device IP from DB")
            continue
        valid.append(dev)
    return valid
...
def _ensure_connected(ip: str) -> bool:
    ...
    try:
        r = subprocess.run(
            [ADB_BIN, "connect", _dev(ip)], capture_output=True, text=True, timeout=15,
        )
...
def _dump_ui(ip: str) -> str:
    ...
    try:
        ...
        r = subprocess.run(
            [ADB_BIN, "-s", _dev(ip), "pull", "/sdcard/ui.xml", str(tmp)],
            capture_output=True,
            text=True,
            timeout=10,
        )
...
def _is_shorts(state: DeviceState, ip: str, xml: str) -> bool:
    ...
    for m in re.finditer(
        ...
        return is_shorts_element(
            player_w,
            player_h,
            screen_w,
            screen_h,
            max_aspect_ratio=min(state.max_aspect_ratio, 1.0),
        )
...
def _force_stop_youtube(ip: str) -> None:
    ...
    for pkg in YOUTUBE_PACKAGES:
        if f"package:{pkg}" in installed:
            subprocess.run(
                [ADB_BIN, "-s", _dev(ip), "shell", "am", "force-stop", pkg],
                capture_output=True,
                timeout=8,
            )
